...
if (
  sessionStorage.getItem("mapStyle") === null ||
  sessionStorage.getItem("mapStyle") === "carto"
) {
  mapTiles =
...
let myMap = L.map(mapContainer, { zoomControl: false }).setView(
  [latitude, longitude],
  13
); //zoom:13
new L.Control.Zoom({
...
if (sessionStorage.getItem("gpsLat") !== null) {
  let gpsLocation = L.marker([latitude, longitude], { icon: gpsIcon }).addTo(
    myMap
  );
...
latUp = latitude + 0.5;
latDown = latitude - 0.5;
//* maximum gps longitude
longLeft = longitude - 0.5;
longRight = longitude + 0.5;
...
if (window.matchMedia("(min-width: 300px) and (max-width: 600px)").matches) {
  if (
    sessionStorage.getItem("menuStatus") === null ||
    sessionStorage.getItem("menuStatus") === "open"
  ) {
    toggle.innerHTML = "&#10096;";
...
mapContainer.addEventListener("contextmenu", (event) => {
  ...
  if (sessionStorage.getItem("gpsStatus") === null) {
    ...
    if (
      ...
      "leaflet-marker-icon leaflet-zoom-animated leaflet-interactive marker-class"
    ) {
      ...
      if (event.clientX + rightClick.offsetWidth > screen.availWidth) {
        rightClick.style.left =
          event.clientX - rightClick.offsetWidth - menuSize + "px";
      ...
      mapContainer.addEventListener(
        "click",
        (e) => (rightClick.style.display = "none")
      );
    }
  }
});
...
let isolatedPlace = () => {
  ...
  rightClick.style.display = "none";
  L.marker([eventLat, eventLong], { icon: isolatedIcon })
    .addTo(myMap)
    ._icon.classList.add("marker-class");
};
...
let badPeople = () => {
  fetch("/badpeople", {
    ...
    .then((returnedID) =>
      returnedID.json().then((data) => badPeopleCount(data))
    )
    .catch((err) => console.log("error " + err));
  //! the then method is working use it update live commits

  rightClick.style.display = "none";
  L.marker([eventLat, eventLong], { icon: badPeopleIcon })
    .addTo(myMap)
    ._icon.classList.add("marker-class");
};
...
let lewdBehaviour = () => {
  fetch("/perverts", {
    ...
    .catch((err) => console.log("error " + err));
  rightClick.style.display = "none";
  L.marker([eventLat, eventLong], { icon: pervertIcon })
    .addTo(myMap)
    ._icon.classList.add("marker-class");
};
...
let rapeZone = async () => {
  if (document.getElementById("checker").checked) {
    ...
    for (let item of rapeData) {
      L.circle([parseFloat(item.latitude), parseFloat(item.longitude)], {
        radius: 10000,
      })
        .setStyle({ color: "red" })
        .addTo(myMap)
